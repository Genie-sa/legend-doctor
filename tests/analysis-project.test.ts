...
test("parses each project file once and caches its source file", () => {
  ...
  assert.strictEqual(second?.sourceFile, first.sourceFile);
...
test("retains syntactic diagnostics on the cached source file", () => {
  ...
  assert.equal(broken.parserDiagnostics[0]?.code, 1109);
  assert.equal(broken.parserDiagnostics[0]?.file, path.resolve("broken.ts"));
...
test("keeps source snapshots independent from later source-map mutations", () => {
  ...
  assert.equal(project.getFile("state.ts")?.sourceFile.text, "export const state = 1;");
...
test("uses filesystem case sensitivity when resolving file identities", () => {
  ...
  if (ts.sys.useCaseSensitiveFileNames) {
    ...
    assert.throws(() => new AnalysisProject(sources), /duplicate analysis file identity/);
  }
});
