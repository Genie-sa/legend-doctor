  ...
  await withProject(
    {
      ...
      "feature/index.ts":
        'export { NamedLeaf } from "./NamedLeaf"; export { default as DefaultLeaf } from "./DefaultLeaf";',
      "screen.tsx":
        'import { DefaultLeaf, NamedLeaf } from "./feature"; export function Screen() { return <><DefaultLeaf /><NamedLeaf /></>; }',
    },
    (root, sources) => {
...
test("resolves only source-proven pure projection wrappers", async () => {
  await withProject(
    {
      "index.ts":
        'export { cx as classNames, noisy, defaulted, shadowed, unknown } from "./projection";',
      "projection.ts": `
    ...
    (root, sources) => {
      const projections = buildSourceIndex(root, sources).pureProjectionsFor(
        path.join(root, "screen.tsx"),
      );
  ...
  await withProject(
    {
      ...
      "screen.tsx":
        'import { Calendar } from "./components/ui/calendar"; import { DropdownMenu } from "./components/ui/dropdown-menu"; export function Screen() { return <><Calendar /><DropdownMenu /></>; }',
    },
    (root, sources) => {
      const components = buildSourceIndex(root, sources).componentsFor(
        path.join(root, "screen.tsx"),
  ...
  await withProject(
    {
      ...
      "apps/native/src/Screen.tsx":
        'import { Row } from "@/Row"; export function Screen() { return <Row />; }',
      "apps/native/tsconfig.json":
        '{"compilerOptions":{"moduleResolution":"Bundler","baseUrl":".","paths":{"@/*":["src/*"]}}}',
      "apps/web/src/Leaf.tsx":
        "export function Leaf({ open }: { open: boolean }) { return open ? <aside /> : null; }",
      "apps/web/src/Panel.tsx": "export function Panel() { return <main />; }",
      "apps/web/src/Screen.tsx":
        'import { Leaf } from "@/Leaf"; import { Panel } from "@/Panel"; export function Screen() { return <><Leaf open={false} /><Panel /></>; }',
...
test("resolves exported Legend observables through aliases and barrels", async () => {
  await withProject(
    ...
    (root, sources) => {
      const observables = buildSourceIndex(root, sources).observablesFor(
...
test("resolves only source-proven observable members from local wrapper factories", async () => {
  await withProject(
    {
      "screen.ts":
        'import { conditional, dialog, namedOnly, reassigned, spread, unknown } from "./state";',
    ...
    (root, sources) => {
      const paths = buildSourceIndex(root, sources).observablePathsFor(
        path.join(root, "screen.ts"),
      );
...
test("does not expose untyped project helpers as observable factories", async () => {
  await withProject(
    ...
    (root, sources) => {
      const factories = buildSourceIndex(root, sources).observableFactoriesFor(
        path.join(root, "screen.ts"),
      );
  ...
  await withProject(
    {
      ...
      `,
      "screen.tsx":
        'import { StyledRoot } from "./Switch"; export function Screen() { return <StyledRoot />; }',
    },
    (root, sources) => {
      const index = buildSourceIndex(root, sources);
      assert.equal(
        index.frameworkEventComponentFor(path.join(root, "Switch.tsx"), "LocalRoot"),
        true,
      );
      assert.equal(
        index.frameworkEventComponentFor(path.join(root, "screen.tsx"), "StyledRoot"),
        true,
      );
    },
  );
...
test("resolves array-valued observable paths across modules, synced initial values, and containers", async () => {
  await withProject(
    {
      "screen.ts": `
        import { pages$ as history$, store$ } from "./state";
        import { library } from "./library";
      `,
      "state.ts": `
        import { observable } from "@legendapp/state";
        import { synced } from "@legendapp/state/sync";
        export const pages$ = observable(synced({ initial: [] as string[] }));
        export const store$ = observable({ rows: [] as string[], meta: { tags: [] as string[], count: 0 } });
      `,
      "library.ts": `
        import { observable } from "@legendapp/state";
        export const library = { todos$: observable<string[]>([]), title$: observable("") };
      `,
    },
    (root, sources) => {
      const paths = buildSourceIndex(root, sources).observableArrayPathsFor(
        path.join(root, "screen.ts"),
      );
