...
async function main(): Promise<void> {
  ...
  for (const repository of repositories) {
    ...
    if (actualCommit !== repository.commit) {
      failures.push(`${repository.name}: expected commit ${repository.commit}, received ${actualCommit}`);
      continue;
    }
  ...
  for (const gold of goldCases) {
    ...
    if (isActionable(finding.action)) {
      actualActionable += 1;
      if (finding.action === gold.action) correctActionable += 1;
    }
    if (isActionable(gold.action)) expectedActionable += 1;
    for (const action of new Set([gold.action, finding.action])) {
      ...
      if (gold.action === action) score.expected += 1;
      if (finding.action === action) score.predicted += 1;
      if (gold.action === action && finding.action === action) score.correct += 1;
...
function sameMembers(
  ...
  return actual.length === expected.length && actual.every((member, index) => member === expected[index]);
}
...
function parseRepositoryRoots(args: readonly string[]): Map<string, string> {
  ...
  for (let index = 0; index < args.length; index += 1) {
    if (args[index] !== "--repo") continue;
    const assignment = args[index + 1];
    if (!assignment) throw new Error("--repo requires name=/absolute/path");
    const separator = assignment.indexOf("=");
    if (separator <= 0) throw new Error(`Invalid repository assignment: ${assignment}`);
...
function findGoldFinding(findings: readonly HookFinding[], gold: (typeof goldCases)[number]): HookFinding | null {
  return (
    findings.find(
      finding =>
