...
856 manual hook labels, including 17 explicit non-enforced opportunities, plus twenty-six
...
Two Formbricks labels verify a persistent dialog payload and visibility flag behind one bounded conditional slot. The
...
Four Tree Map labels verify two payload/open modal models behind bounded logical-AND gates. The stable wrapper replaces
the complete `payload && <Modal />` slot; a close-complete callback may clear the payload after the open leaf closes.
Multiple payload gates remain a hard negative.
...
snapshots, and only the dialog wrapper subscribes. Payload fanout, functional updaters,
effects, unresolved events, broad dialogs, and coupled writes abstain.

One Tree Map label extends that single-state proof to a bounded conditional dialog with call-free payload projections
inside the selected child. The always-mounted subscriber replaces the complete conditional slot, preserving the dialog's
conditional mount identity. Payload fanout, impure projections, repeated output, broad branches, unresolved events,
callable payloads, effects, and coupled writes abstain.
...
One Excalidraw label resolves the true branch of a boolean gate through an immutable local zero-argument JSX factory.
The factory must have one direct JSX return. Mutable bindings, parameters, wrapper-produced functions, and multiple
returns abstain, while the adjacent payload-coupled dialog remains `review-state`.
...
literal or literal default. This is callsite proof, not a `Button` or `asChild` convention. The proof may cross a chain of
rest-prop JSX spreads when each rest binding is immutable, has no non-JSX use, and its outer invocation resolves the same
...
Seventeen non-enforced async labels record safe opportunities behind callback chains that the analyzer does not yet prove.
...
array escapes and render-time item callback calls remain review. One Open WebUI label follows a conditionally selected
callback through two transparent rest-prop objects into React Native Pressable; calling that tracked callback in either
branch remains review. The remaining labels include unresolved local wrapper stacks, opaque prop spreads, and Radix
