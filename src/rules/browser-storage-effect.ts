...
import { type RuntimeFunctionLike, visit } from "../ast.js";
...
export function isDependencyDrivenBrowserStorageEffect(effect: EffectCandidate): boolean {
  ...
  const statementIsSafe = (statement: ts.Statement): boolean => {
    ...
    if (ts.isIfStatement(statement)) {
      return isSafeStorageExpression(statement.expression, owner) &&
    ...
    if (!ts.isCallExpression(expression) || !isBrowserStorageMutation(expression, owner)) return false;
    if (!expression.arguments.every(argument => isSafeStorageExpression(argument, owner))) return false;
    storageMutations += 1;
    return true;
  };
...
function isSafeStorageExpression(
  ...
  visit(expression, node => {
    ...
    if (ts.isCallExpression(node)) {
      if (!isPureStorageHelper(node, owner)) safe = false;
      return;
    }
    if (
...
function isPureStorageHelper(
  ...
  return bindingDeclarationCount(owner, root) === 0 &&
    ((root === "JSON" && method === "stringify") ||
      (root === "Object" && method === "keys"));
}

function isBrowserStorageMutation(
  call: ts.CallExpression,
  owner: RuntimeFunctionLike
): boolean {
  const callee = call.expression;
...
function isBrowserStorageExpression(
  ...
  if (ts.isIdentifier(value)) {
    return /^(?:localStorage|sessionStorage)$/.test(value.text) &&
  ...
  if (ts.isIdentifier(qualifier)) {
    return /^(?:globalThis|window)$/.test(qualifier.text) &&
      bindingDeclarationCount(runtimeOwner, qualifier.text) === 0;
  }
  return ts.isPropertyAccessExpression(qualifier) &&
