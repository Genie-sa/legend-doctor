...
export function keyedCursorConsumerResult(
  ...
  if (declaresRuntimeBinding(sourceFile, hookBinding)) return "unsafe";
  ...
  if (calls.length === 0 && !unsafeReference) return "none";
  if (unsafeReference || calls.length !== 1) return "unsafe";
...
function callHasStableKeyedCursorConsumer(
  ...
  const element = declaration.name.elements.find(candidate => {
    const sourceName = candidate.propertyName && ts.isIdentifier(candidate.propertyName)
...
function cursorReferencesFormOneStableList(
  ...
  if (equalityReferences.length !== 1) return false;
  ...
  if (!ts.isCallExpression(memo) || memo.arguments[0] !== callback || !isImportedUseCallback(memo)) {
    return false;
  }
  ...
  if (!dependency || !ts.isArrayLiteralExpression(dependency)) return false;
  const dependencyReferences = references.filter(reference => nodeWithin(reference, dependency));
  if (
    dependencyReferences.length !== 1 ||
    !dependency.elements.some(element => unwrapTransparentExpression(element) === dependencyReferences[0])
  ) {
    return false;
  }

  const extraDataReferences = references.filter(reference =>
    jsxAttributeContaining(reference, "extraData") !== null
  );
  if (extraDataReferences.length !== 1) return false;
  if (
  ...
  return !!keyAttribute && ts.isJsxAttribute(keyAttribute) && keyExtractorIsStable(keyAttribute, owner);
}
...
function keyExtractorIsStable(
  ...
  if (!callback || callback.parameters.length === 0 || !ts.isIdentifier(callback.parameters[0]!.name)) {
    return false;
  }
  ...
  if (!returned || !isPureExpression(returned)) return false;
  let itemProperty = false;
  let usesIndex = false;
  visit(returned, node => {
    if (
      ts.isPropertyAccessExpression(node) &&
      propertyAccessRoot(node)?.text === item.text
    ) {
      itemProperty = true;
    }
...
function callbackFromExpression(
  ...
  if (!ts.isCallExpression(initializer) || !isImportedUseCallback(initializer)) return null;
...
function cursorEquality(reference: ts.Identifier): ts.BinaryExpression | null {
  ...
  return ts.isBinaryExpression(parent) &&
    ...
    [ts.SyntaxKind.EqualsEqualsToken, ts.SyntaxKind.EqualsEqualsEqualsToken].includes(parent.operatorToken.kind)
    ? parent
    : null;
}
...
function jsxAttributeContaining(
  node: ts.Node,
  name: string
): ts.JsxAttribute | null {
