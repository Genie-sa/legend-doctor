...
export class ReactCompilerResolver {
  ...
  public compilesDirectory(directory: string): Promise<boolean> {
    return this.directoryCompiles(path.resolve(directory));
  }
...
async function configEnablesReactCompiler(directory: string): Promise<boolean> {
  ...
  return anyFileMarksCompiler(candidates.map((name) => path.join(directory, name)));
}
