...
interface ExecutionPath {
  awaitEpoch: number;
  events: Array<{ call: ts.CallExpression; epoch: number }>;
...
class FunctionWriteFlow {
  ...
  prove(left: ts.CallExpression, right: ts.CallExpression): FlowProof {
    ...
    if (haveOppositeSharedArm(leftControls, rightControls)) return "disproven";
    if (
    ...
    const together = result.paths.filter(path =>
      path.events.some(event => event.call === left) && path.events.some(event => event.call === right)
    );
...
function lowerStatements(
  ...
  for (const statement of statements) {
    ...
    if (paths.length > MAX_PATHS) return { paths: paths.slice(0, MAX_PATHS), unknown: true };
  }
...
function lowerStatement(
  ...
  if (ts.isVariableStatement(statement)) {
    ...
    for (const declaration of statement.declarationList.declarations) {
      if (!declaration.initializer || isRuntimeFunctionLike(declaration.initializer)) continue;
  ...
  if (ts.isIfStatement(statement)) {
    ...
    const thenResult = lowerStatement(statement.thenStatement, condition.paths, context, left, right);
  ...
  if (ts.isReturnStatement(statement) || ts.isThrowStatement(statement)) {
    ...
    return { paths: result.paths.map(path => ({ ...path, termination: "return" })), unknown: result.unknown };
  }
  if (ts.isBreakStatement(statement)) {
    if (statement.label || !context.breakable) return { paths: clonePaths(incoming), unknown: true };
...
function lowerSwitch(
  ...
  let unknown = discriminant.unknown;
  ...
  for (let start = 0; start < clauses.length; start += 1) {
    ...
    for (let index = start; index < clauses.length; index += 1) {
      ...
      if (!clause) continue;
      if (ts.isCaseClause(clause) && !isInertCaseExpression(clause.expression)) unknown = true;
  ...
  if (!clauses.some(ts.isDefaultClause)) outputs.push(...clonePaths(discriminant.paths));
...
function lowerExpression(
  ...
  if (isRuntimeFunctionLike(expression)) return { paths: clonePaths(incoming), unknown: false };
  ...
  if (ts.isCallExpression(expression)) {
    if (expression.questionDotToken) return { paths: clonePaths(incoming), unknown: true };
    let result = lowerExpression(expression.expression, incoming, left, right);
    for (const argument of expression.arguments) {
      if (isRuntimeFunctionLike(argument)) continue;
...
function haveOppositeSharedArm(left: readonly ControlArm[], right: readonly ControlArm[]): boolean {
  return left.some(leftArm =>
    leftArm.exclusive && right.some(rightArm =>
      rightArm.exclusive &&
...
function sameControlArms(left: readonly ControlArm[], right: readonly ControlArm[]): boolean {
  return left.length === right.length && left.every(leftArm =>
    right.some(rightArm => leftArm.control === rightArm.control && leftArm.arm === rightArm.arm)
  );
}
...
function shareSwitchControl(left: readonly ControlArm[], right: readonly ControlArm[]): boolean {
  return left.some(leftArm =>
    !leftArm.exclusive && right.some(rightArm =>
      !rightArm.exclusive && leftArm.control === rightArm.control
    )
  );
}
...
function unconditionalCallPrecedesControlledCall(
  ...
  return paths.every(path => {
    ...
    return uncontrolledIndex >= 0 && uncontrolledIndex < controlledIndex;
  });
}
...
function callsShareAwaitEpoch(
  ...
  for (const event of path.events) {
    if (event.call === left) leftEpoch = event.epoch;
    if (event.call === right) rightEpoch = event.epoch;
  }
...
function nodeWithinFunction(node: ts.Node, fn: RuntimeFunctionLike): boolean {
  for (let current: ts.Node | undefined = node; current; current = current.parent) {
    ...
    if (current !== node && isRuntimeFunctionLike(current)) return false;
  }
  return false;
}
...
function constantBoolean(expression: ts.Expression): boolean | null {
  ...
  if (ts.isBigIntLiteral(expression)) return expression.text !== "0n";
  if (
...
function shortCircuitRightExecution(
...
function constantNullish(expression: ts.Expression): boolean | null {
  ...
  return ts.isParenthesizedExpression(expression)
    ? constantNullish(expression.expression)
    : null;
}
